...
class Heap {
    ...
    insert(node) {
        ...
        let parent = point / 2 | 0;
        while (parent > 0 && store[point].sortNum > store[parent].sortNum) { // 自下往上堆化
            HeapUtil.swap(store, point, parent); // swap()函數作用：交換下標為i和i/2的兩個元素
            point = parent;
            parent = parent / 2 | 0;
        }
    }
...
function HeapTest() {
    ...
    console.log('check:', isHeapArr(maxHeap.PRIVATE.store));
    console.log('獲取最大值：', maxHeap.getMax());
    console.log('check:', isHeapArr(maxHeap.PRIVATE.store));
    console.log('獲取最大值：', maxHeap.getMax());
    console.log('check:', isHeapArr(maxHeap.PRIVATE.store));
